...
/**
 * Base class for tests that exercise the UI code path via PlayerControllerForTests.
 * Mirrors AITest's helper methods but uses LobbyPlayerForTests instead of LobbyPlayerAi,
 * so all player decisions flow through the same code as PlayerControllerHuman.
 */
public class UIPathTest {
    ...
    public Game resetGame() {
        ...
        Deck d1 = new Deck();
        players.add(new RegisteredPlayer(d1).setPlayer(new LobbyPlayerForTests("p2", null)));
        players.add(new RegisteredPlayer(d1).setPlayer(new LobbyPlayerForTests("p1", null)));
    ...
    protected PlayerControllerForTests getController(Player p) {
        return (PlayerControllerForTests) p.getController();
    }

    protected boolean castSpell(Player p, SpellAbility sa) {
        sa.setActivatingPlayer(p);
        return p.getController().playSaFromPlayEffect(sa);
    }
